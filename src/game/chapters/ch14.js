...
import {
  ...
  note, heart, mira, rng, mix, clamp, easeOut, easeInOut, lerp,
} from '../paint.js'
...
function season(ctx, m, w, h) {
  ...
  if (m >= 3) {
    for (let k = 0; k < (m === 3 ? 6 : 14); k++) {
      const x = 30 + ((k * 97) % (w - 60))
      const y = 30 + ((k * 53) % (h - 60))
...
const calendar = (api) => {
  ...
  return {
    ...
    draw(ctx, t) {
      ...
      plant(ctx, 440, 830, 1.1, G('#a9b88f'), 1403)
      wash(ctx, 80, 770, 50, 60, G(C.sky), 1404) // a mug
...
const NIGHT = (c) => mix(c.startsWith('#') ? c : '#888888', '#2c2d48', 0.45)

// A head on a pillow seen from above, lying on its side. face: -1 left, 1 right.
function head(ctx, x, y, face, hair, skin, bun, asleep) {
  blob(ctx, x, y, 34, 40, skin, 1450 + x)
  ctx.fillStyle = hair
  ctx.beginPath()
  ctx.ellipse(x - face * 10, y - 4, 32, 40, 0, 0, Math.PI * 2)
  ctx.fill()
  if (bun) {
    ctx.beginPath()
    ctx.arc(x - face * 34, y - 22, 15, 0, Math.PI * 2)
    ctx.fill()
  }
  ctx.fillStyle = skin
  ctx.beginPath()
  ctx.ellipse(x + face * 14, y + 4, 20, 30, 0, 0, Math.PI * 2)
  ctx.fill()
  ctx.strokeStyle = C.ink
  ctx.fillStyle = C.ink
  ctx.lineWidth = 3
  ctx.beginPath()
  if (asleep) ctx.arc(x + face * 20, y - 4, 5, 0.2, Math.PI - 0.2)
  else ctx.arc(x + face * 20, y - 6, 3.5, 0, Math.PI * 2)
  if (asleep) ctx.stroke()
  else ctx.fill()
}
...
function glowPhone(ctx, x, y, lit) {
  if (lit > 0) {
    ...
    g.addColorStop(0, `rgba(190,220,240,${0.45 * lit})`)
  ...
  ctx.fillRect(x - 12, y - 22, 24, 44)
}

const bed = (api) => {
  const HAND0 = 222
  const HAND1 = 300 // his back starts at ~316: she stops just short
  let holding = false
  let reach = 0
  let reachedAt = null
  let doneAt = null
  return {
    debug: () => ({ hold: [215, 340], reach, done: doneAt !== null }),
    draw(ctx, t, dt) {
      if (reachedAt === null) {
      ...
      wash(ctx, 40, 100, 460, 840, NIGHT('#6e5a50'), 1461)
      wash(ctx, 60, 130, 420, 790, NIGHT('#e4e0ea'), 1462)
      wash(ctx, 80, 160, 170, 100, NIGHT('#f4f1ea'), 1463)
      wash(ctx, 290, 160, 170, 100, NIGHT('#f4f1ea'), 1464)
      head(ctx, 160, 220, -1, C.miraHair, NIGHT(C.skin1), true, false)
      head(ctx, 380, 220, 1, C.arunHair, NIGHT(C.skin2), false, hisLight < 0.5)
      glowPhone(ctx, 96, 262, 1)
      // his phone slips down onto the pillow once he's gone to sleep
      glowPhone(ctx, 448, 262 + (1 - hisLight) * 26, hisLight)
      blob(ctx, 104, 290, 14, 12, NIGHT(C.skin1), 1465)
      if (hisLight > 0.5) blob(ctx, 440, 290, 14, 12, NIGHT(C.skin2), 1466)
      // blanket with two humps and a valley between them
      wash(ctx, 60, 290, 420, 620, NIGHT(G('#8d95b0')), 1467)
      blob(ctx, 170, 520, 100, 220, NIGHT(G('#7c84a0')), 1468)
      blob(ctx, 370, 520, 100, 220, NIGHT(G('#7c84a0')), 1469)
      line(ctx, 270, 330, 272, 880, 'rgba(20,20,40,0.35)', 4, 7)
      blob(ctx, 170, 300, 58, 30, NIGHT(G(C.mira)), 1470)
      blob(ctx, 372, 300, 58, 30, NIGHT(G('#50627a')), 1471)
      if (reach > 0) {
        const hx = lerp(HAND0, HAND1, easeOut(reach))
        line(ctx, 200, 336, hx - 12, 342, NIGHT(G(C.mira)), 24, 8)
        blob(ctx, hx, 342, 15, 12, NIGHT(C.skin1), 1472)
      }
      if (reachedAt !== null) {
        for (let k = 0; k < 3; k++) {
          const q = ((t - reachedAt) * 0.4 + k / 3) % 1
          text(ctx, 'z', 420 + q * 40, 170 - q * 80, { size: 22 + k * 4, color: C.cream, alpha: (1 - q) * 0.8 })
        }
      }
      if (doneAt === null) {
        if (reachedAt === null) {
          text(ctx, 'press and hold to reach out', W / 2 + 20, 60, { size: 30, color: C.cream })
          if (!holding) tapHint(ctx, 215, 340, t, C.cream)
        }
      } else {
...
const morning = vignette((ctx, t) => {
  panel(ctx, 30, 100, 480, 400, (ctx, w, h) => {
    ...
    wash(ctx, 150, 240, 320, 90, G('#e9e3f0'), 1493) // the bed
    wash(ctx, 360, 222, 90, 40, '#f4f1ea', 1494)
    blob(ctx, 405, 250, 10, 6, G(C.greyLight), 1495) // a dent where he slept
    wash(ctx, 382, 232, 34, 22, '#fbf8f1', 1496)
    mira(ctx, 120, 400, { pose: 'sit', eyes: 'down', grey: GREY })
  })
  panel(ctx, 30, 540, 480, 250, (ctx, w, h) => {
    ...
    ctx.lineWidth = 3
    ctx.beginPath()
    ctx.ellipse(-90, 20, 22, 26, 0, 0, Math.PI * 2)
    ctx.moveTo(-90, -6)
    ctx.lineTo(-90, -60)
    ctx.moveTo(-40, 0)
    ctx.lineTo(20, 0)
